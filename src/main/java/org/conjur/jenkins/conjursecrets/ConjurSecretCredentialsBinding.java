	...
	private final @Nonnull <C> C getCredentialsForBind(@Nonnull Run<?, ?> build) throws IOException {
		...
		if (cred == null) {
			...
			if (item != null) {
				...
				conjurSecretCredential = ConjurSecretCredentials.credentialWithID(newCredentialId, item); /*Passing item instead of build to fix NetAPP Null Pointer Issue Modified on 3rd Aug 2023 Auth:Sameer*/
